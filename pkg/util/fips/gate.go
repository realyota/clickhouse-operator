...
//     The Go runtime mode (`GODEBUG=fips140=on` default vs `=only` strict,
//     exposed via `runtime`) is independent — `=on` is sufficient for FIPS
//     compatibility; `=only` is opt-in defense-in-depth.
...
func EvaluateGate(binaryName string, enabled, build, runtime bool) (err error, warn string) {
	switch {
	case enabled && !build:
